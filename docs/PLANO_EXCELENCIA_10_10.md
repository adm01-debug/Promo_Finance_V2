...
- [~] **#7** Zod contracts nas 94 Edge Functions. Baseline **52 → 45 → 39** (-13). Cobertas as 6 edges do motor tributário (`simular-real`, `simular-presumido`, `simular-simples`, `decidir-regime`, `projecao-reforma`, `prever-carga-tributaria`) além das 7 anteriores (SEFAZ/SPED/OCR/LGPD). Schemas `ParametrosSimulacaoSchema`, `SimularSimplesRpcSchema`, `DecidirRegimeSchema` e `ProjecaoReformaSchema` centralizados em `_shared/validation.ts`. Gate CI atualizado. Próxima onda: SSO, n8n dispatchers e notifiers.
