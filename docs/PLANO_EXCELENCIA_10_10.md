...
- [~] **#11** Rate limit universal via `_shared/rate-limit.ts`: já cobre Asaas/Bling/Bitrix24 webhooks + endpoints IA de alto custo (`analyze-document`, `expert-agent`, `categorizar-despesa`, janela 20-30 req/min por IP). Restante: `whatsapp-ia-proativo`, `insights-relatorio`, `benchmarking-setorial` e demais AI endpoints.
