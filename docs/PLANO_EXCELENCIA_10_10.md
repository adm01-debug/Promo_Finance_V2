...
- [~] **#7** Zod contracts nas 94 Edge Functions. Baseline **52 → 45 → 39 → 29** (-23). Coberturas por onda: SEFAZ/SPED/OCR/LGPD (7), motor tributário (6), SSO (4: `sso-initiate`, `sso-generate-metadata`, `sso-test-login`, `sso-validate-config`), integração externa (2: `n8n-dispatch`, `n8n-callback`) e notifiers (4: `notify-performance-alert`, `notify-saved-filter`, `send-device-alert`, `send-push-notification`). Typecheck limpo, gate CI verde em 29. Próxima onda: copilots (`copilot-global`, `copilot-tributario`), IA (`analise-fluxo-ia`, `conciliacao-ia`, `whatsapp-ai-analyzer`) e relatórios (`gerar-*`, `insights-relatorio`).
