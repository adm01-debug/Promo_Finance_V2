...
- [~] **#4** Modularizar 19 arquivos > 700 linhas. Progresso: `AnomaliasDetectadasPanel` **1210 → 324** ✅, `Asaas.tsx` **1186 → 559** ✅, `DreBalancoTab` **1021 → 235** ✅, `ContabilizacaoAutomaticaTab` **1036 → 156** ✅, `SpedEcfWizard.tsx` **950 → 138** ✅ (extraído em `sped-ecf-wizard/`: `types`, `wizard-atoms` (StepPills/MetaField/KpiChip/KpiCard), `useSpedEcfWizardState` (persistência localStorage + handlers), `Step1PeriodoEcd`, `Step2Validacoes`, `Step3Download` (cross-check ECF×ECD + goToAnchor). Comportamento, âncoras e chave de rascunho preservados byte-a-byte). `tsgo` limpo. Próximo alvo a inspecionar entre os remanescentes > 700 linhas.
