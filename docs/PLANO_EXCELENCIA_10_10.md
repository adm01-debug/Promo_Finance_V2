...
- [~] **#7** Zod contracts nas 94 Edge Functions. Baseline **52 → 45** (-7). Aplicado `validatePayload` + schemas discriminados nas 7 funções críticas SEFAZ/SPED/OCR/LGPD: `exportar-sped-contribuicoes`, `gerar-sped-ecd`, `gerar-sped-ecf`, `log-sped-bitrix24`, `processar-nf-ocr`, `processar-solicitacao-lgpd`, `sefaz-manifestar`. Gate `scripts/security/zod-coverage.sh` atualizado no baseline. Próxima onda: cron dispatchers (`gerar-alertas-*`, `analise-fluxo-ia`) e proxies SSO.
