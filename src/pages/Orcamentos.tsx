        ...
        <div className="container mx-auto p-6 relative z-10">
          ...
          >
            <div className="flex items-center gap-3">
              <Input 
                ...
                className="w-40 bg-card/5 border-border text-foreground"
              />
              <Button onClick={() => {
                setEditingBudget(null);
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
            <Card className="bg-card/5 border-border backdrop-blur-xl">
              <CardContent className="pt-6">
                ...
                <div className="text-2xl font-black text-foreground">{formatCurrency(totals.totalBudgeted)}</div>
                <p className="text-xs text-foreground/40 mt-1">Planejado para {selectedPeriod}</p>
              </CardContent>
            </Card>
            ...
            <Card className="bg-card/5 border-border backdrop-blur-xl">
              <CardContent className="pt-6">
                ...
                <div className="text-2xl font-black text-foreground">{formatCurrency(totals.totalSpent)}</div>
                <p className="text-xs text-foreground/40 mt-1">{totals.percent.toFixed(1)}% do orçamento consumido</p>
              </CardContent>
            </Card>
            ...
            <Card className="bg-card/5 border-border backdrop-blur-xl">
              <CardContent className="pt-6">
                ...
                <div className="text-2xl font-black text-foreground">{formatCurrency(totals.remaining)}</div>
                <p className="text-xs text-foreground/40 mt-1">Saldo disponível para o período</p>
              </CardContent>
            </Card>
            ...
            <Card className="bg-card/5 border-border backdrop-blur-xl">
              <CardContent className="pt-6">
                ...
                <div className="text-2xl font-black text-foreground">{totals.percent.toFixed(0)}%</div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            <div className="lg:col-span-2 space-y-6">
              <Card className="bg-card/5 border-border backdrop-blur-xl">
                <CardHeader>
                  <CardTitle className="text-foreground">Orçamentos por Categoria</CardTitle>
                  ...
                  <div className="space-y-6">
                    {budgets.length === 0 ? (
                      ...
                      budgets.map((budget: any) => {
                        ...
                        return (
                          <div key={budget.id} className="group p-4 rounded-2xl bg-card/5 border border-white/5 hover:border-border transition-all">
                            <div className="flex justify-between items-start mb-4">
                              <div>
                                ...
                                <p className="text-xs text-foreground/40">Periodo: {budget.period}</p>
                              </div>
                              <div className="flex items-center gap-2">
                                <Button variant="ghost" size="icon" onClick={() => handleEdit(budget)} className="h-8 w-8 text-foreground/40 hover:text-foreground">
                                  <Pencil className="h-4 w-4" />
                                </Button>
                                <Button variant="ghost" size="icon" onClick={() => deleteBudget.mutate(budget.id)} className="h-8 w-8 text-foreground/40 hover:text-destructive">
                            ...
                            <div className="grid grid-cols-3 gap-4 mb-3">
                              <div>
                                <p className="text-[10px] uppercase text-foreground/40 font-bold mb-1">Orçado</p>
                                <p className="text-sm font-bold text-foreground">{formatCurrency(budget.budgeted_amount)}</p>
                              </div>
                              <div>
                                <p className="text-[10px] uppercase text-foreground/40 font-bold mb-1">Gasto</p>
                                <p className="text-sm font-bold text-foreground">{formatCurrency(budget.actual_spent)}</p>
                              </div>
                              <div className="text-right">
                                <p className="text-[10px] uppercase text-foreground/40 font-bold mb-1">Status</p>
            ...
            <div className="space-y-6">
              <Card className="bg-card/5 border-border backdrop-blur-xl">
                <CardHeader>
                  <CardTitle className="text-foreground">Análise de Gastos</CardTitle>
                </CardHeader>
                <CardContent>
              ...
              <Card className="bg-primary/5 border-primary/20 backdrop-blur-xl border-dashed">
                <CardContent className="pt-6">
                  <div className="flex items-center gap-3 mb-4">
                    ...
                    <h4 className="font-bold text-foreground">Insight Quantum</h4>
                  </div>
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent className="bg-popover border-border text-foreground max-w-md">
          ...
          <form onSubmit={form.handleSubmit} className="space-y-6 py-4">
            ...
            <DialogFooter>
              <Button type="button" variant="ghost" onClick={() => setIsDialogOpen(false)} className="text-foreground/40">
                Cancelar
              </Button>
