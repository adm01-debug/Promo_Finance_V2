...
export default function DashboardEmpresa() {
  ...
  const { data: kpis } = useDashboardKPIs(
    selectedEmpresa || (empresas.length > 0 ? empresas[0].id : '')
  );
  ...
  const totalVencidasReceber = contasReceberEmpresa
    .filter((c) => c.status === 'vencido')
    .reduce((sum, c) => sum + c.valor - (c.valor_recebido || 0), 0);
  const totalVencidasPagar = contasPagarEmpresa
    .filter((c) => c.status === 'vencido')
    .reduce((sum, c) => sum + c.valor - (c.valor_pago || 0), 0);
  ...
  const statusReceberData = useMemo(() => {
    ...
    return [
      { name: 'Recebidas', value: counts.pago, fill: 'var(--ok)' },
      { name: 'Pendentes', value: counts.pendente, fill: 'var(--warn)' },
      { name: 'Vencidas', value: counts.vencido, fill: 'var(--bad)' },
      { name: 'Parciais', value: counts.parcial, fill: 'var(--acc)' },
    ].filter((s) => s.value > 0);
  ...
  const statusPagarData = useMemo(() => {
    ...
    return [
      { name: 'Pagas', value: counts.pago, fill: 'var(--ok)' },
      { name: 'Pendentes', value: counts.pendente, fill: 'var(--warn)' },
      { name: 'Vencidas', value: counts.vencido, fill: 'var(--bad)' },
      { name: 'Parciais', value: counts.parcial, fill: 'var(--acc)' },
    ].filter((s) => s.value > 0);
  ...
  const topClientesReceber = useMemo(() => {
    ...
    contasReceberEmpresa
      .filter((c) => c.status !== 'pago' && c.status !== 'cancelado')
      .forEach((c) => {
        if (!map.has(c.cliente_nome))
          map.set(c.cliente_nome, { nome: c.cliente_nome, valor: 0, vencido: 0 });
        const cur = map.get(c.cliente_nome)!;
        cur.valor += c.valor - (c.valor_recebido || 0);
        if (c.status === 'vencido') cur.vencido += c.valor - (c.valor_recebido || 0);
      });
  ...
  const topFornecedoresPagar = useMemo(() => {
    ...
    contasPagarEmpresa
      .filter((c) => c.status !== 'pago' && c.status !== 'cancelado')
      .forEach((c) => {
        if (!map.has(c.fornecedor_nome))
          map.set(c.fornecedor_nome, { nome: c.fornecedor_nome, valor: 0, vencido: 0 });
        const cur = map.get(c.fornecedor_nome)!;
        cur.valor += c.valor - (c.valor_pago || 0);
        if (c.status === 'vencido') cur.vencido += c.valor - (c.valor_pago || 0);
      });
    return Array.from(map.values())
      .sort((a, b) => b.valor - a.valor)
      .slice(0, 5);
  ...
  return (
    <MainLayout>
      ...
      >
        ...
        >
          <div>
            ...
            <p className="text-muted-foreground mt-1">
              Análise detalhada com drill-down financeiro
            </p>
          </div>
        ...
        <EmpresaChartsSection
          fluxoCaixaProjetado={fluxoCaixaProjetado}
          statusReceberData={statusReceberData}
          statusPagarData={statusPagarData}
          periodoAnalise={periodoAnalise}
        />
        <EmpresaDrillDownSection
          contasBancarias={contasBancariasEmpresa}
          topClientesReceber={topClientesReceber}
          topFornecedoresPagar={topFornecedoresPagar}
