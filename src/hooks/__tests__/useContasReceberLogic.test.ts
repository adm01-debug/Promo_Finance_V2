...
describe('useContasReceberLogic - KPI calculation', () => {
  ...
  const createConta = (overrides: any = {}) => ({
    ...
    valor_recebido: 0,
    status: 'pendente',
    data_vencimento: toLocalYMD(today),
  ...
  describe('Gap #5 - Cálculo vence hoje', () => {
    it('identifica contas que vencem hoje', () => {
      const contas = [
        createConta({ data_vencimento: toLocalYMD(today) }),
    ...
    it('exclui contas pagas do vence hoje', () => {
      const contas = [createConta({ data_vencimento: toLocalYMD(today), status: 'pago' })];
      const venceHoje = contas.filter((c) => {
        if (c.status === 'pago' || c.status === 'cancelado') return false;
        const venc = parseLocalDate(c.data_vencimento);
        venc.setHours(0, 0, 0, 0);
  ...
  describe('Gap #5 - Cálculo vence semana', () => {
    it('identifica contas que vencem nos próximos 7 dias', () => {
      ...
      const contas = [
        createConta({ data_vencimento: toLocalYMD(d3) }),
        createConta({ data_vencimento: toLocalYMD(d10) }),
      ];
      ...
      const total = contas.reduce(
        (sum, c) =>
          c.status !== 'pago' && c.status !== 'cancelado'
            ? sum + c.valor - (c.valor_recebido || 0)
            : sum,
        0
      );
  ...
  describe('Gap #3 - Filtro empresa', () => {
    ...
    it('retorna todas quando filtro é "all"', () => {
      const contas = [createConta({ empresa_id: 'emp-1' }), createConta({ empresa_id: 'emp-2' })];
      const empresaFilter = 'all';
      const filtered = contas.filter(
        (c) => empresaFilter === 'all' || c.empresa_id === empresaFilter
      );
      expect(filtered.length).toBe(2);
    });
  });
