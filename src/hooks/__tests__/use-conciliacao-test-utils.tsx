...
/** Shape mínimo do match de alta confiança usado nos testes de importação
 * (mesmos campos consumidos por useConciliacaoPage/aplicarConciliacoesAutomaticas). */
interface MatchSugestaoAltaConfianca {
  lancamentoId: string;
  lancamentoTipo: 'receber' | 'pagar';
  lancamento: { id: string; valor: number };
  confianca: 'alta' | 'media' | 'baixa';
  score: number;
  motivos: { tipo: string; descricao: string; peso: number }[];
}

/** Match de alta confiança padrão (fixture repetido nos testes de importação). */
export function makeMatchAltaConfianca(
  tipo: 'receber' | 'pagar' = 'receber'
): Map<string, MatchSugestaoAltaConfianca[]> {
  return new Map<string, MatchSugestaoAltaConfianca[]>([
    [
      'tx-1',
      [
        {
