...
const PESOS_MODELO = {
  ...
  frequenciaPagamento: 0.1,
  tendenciaPagamento: 0.1,
...
export function useScoringClientes() {
  ...
  const { data: clientesComScore = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data: contasReceber, error: contasError } = await supabase
        .from('contas_receber')
        .select(
          'cliente_id, valor, valor_recebido, data_vencimento, data_recebimento, status, created_at'
        )
      ...
      const clientesProcessados: ClienteScore[] = (clientes || []).map((cliente) => {
        ...
        const limiteBase =
          historico.valorTotal > 0
            ? historico.valorTotal / Math.max(historico.totalTitulos, 1)
            : 5000;
  ...
  const recalcularScoreMutation = useMutation({
    mutationFn: async (clienteId: string) => {
      ...
      await supabase.from('clientes').update({ score: cliente.score }).eq('id', clienteId);
  ...
  const estatisticas = {
    ...
    scoreMedio:
      clientesComScore.length > 0
        ...
        : 0,
    clientesRiscoAlto: clientesComScore.filter((c) => c.risco === 'alto' || c.risco === 'critico')
      .length,
...
function calcularHistorico(contas: ContaReceberScoring[]): DadosHistorico {
  ...
  const titulosVencidos = contas.filter(
    (c) => c.status !== 'pago' && new Date(c.data_vencimento) < hoje
  ).length;
  ...
  const primeiraCompra = contas.length > 0 ? new Date(contas[contas.length - 1].created_at) : hoje;
...
function calcularScore(historico: DadosHistorico): {
  ...
  const taxaPagamento =
    historico.totalTitulos > 0 ? (historico.titulosPagos / historico.totalTitulos) * 100 : 100;
  ...
  fatores.push({
    ...
    impacto:
      historico.diasMedioAtraso <= 5
        ? 'positivo'
        : historico.diasMedioAtraso <= 15
          ? 'neutro'
          : 'negativo',
  ...
  fatores.push({
    ...
    impacto:
      historico.tempoRelacionamento >= 12
        ? 'positivo'
        : historico.tempoRelacionamento >= 6
          ? 'neutro'
          : 'negativo',
  ...
  fatores.push({
    ...
    impacto:
      historico.valorTotal >= 50000
        ? 'positivo'
        : historico.valorTotal >= 10000
          ? 'neutro'
          : 'negativo',
  ...
  const penalizacaoAtrasoGrave =
    historico.maiorAtraso > 60 ? 100 : historico.maiorAtraso > 30 ? 50 : 0;
  ...
  fatores.push({
    ...
    impacto:
      penalizacaoAtrasoGrave === 0
        ? 'positivo'
        : penalizacaoAtrasoGrave < 50
          ? 'neutro'
          : 'negativo',
    descricao:
      historico.maiorAtraso > 0
        ? `Maior atraso: ${historico.maiorAtraso} dias`
        : 'Sem atrasos graves',
  });
  ...
  const score = Math.min(
    ...
    Math.max(
      0,
      fatores.reduce((s, f) => s + f.valor, 0)
    )
  );

  // Classificação
  const classificacao: 'A' | 'B' | 'C' | 'D' | 'E' =
    score >= 800 ? 'A' : score >= 600 ? 'B' : score >= 400 ? 'C' : score >= 200 ? 'D' : 'E';

  // Risco
  const risco: 'baixo' | 'medio' | 'alto' | 'critico' =
    score >= 700 ? 'baixo' : score >= 500 ? 'medio' : score >= 300 ? 'alto' : 'critico';
...
function calcularTendencia(contas: ContaReceberScoring[]): 'subindo' | 'estavel' | 'descendo' {
  ...
  const taxaRecente =
    contasRecentes.filter((c) => c.status === 'pago').length / contasRecentes.length;
