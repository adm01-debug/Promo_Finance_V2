...
export function useApuracoesTributarias(empresaId?: string) {
  ...
  const atualizarApuracao = useMutation({
    mutationFn: async ({
      ...
      Partial<ApuracaoTributaria>,
      'id' | 'data_transmissao' | 'protocolo_transmissao' | 'created_by'
    > & { id: string }) => {
      const { data, error } = await supabase
        .from('apuracoes_tributarias')
        // TODO(2026-08-14): data_transmissao/protocolo_transmissao/created_by excluídos — não existem em apuracoes_tributarias (types.ts)
        .update(dados)
        .eq('id', id)
        .select()
        .single();
  ...
  const calcularApuracao = useMutation({
    mutationFn: async ({
      ...
      const total_tributos_residuais =
      ...
      const { data, error } = await supabase
        ...
        .update({
          ...
          pis_residual,
          cofins_residual,
          total_tributos_novos,
          total_tributos_residuais,
          total_geral,
          status: 'calculado',
        })
        .eq('id', id)
