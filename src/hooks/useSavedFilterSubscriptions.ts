...
export function useSavedFilterSubscriptions() {
  ...
  useEffect(() => {
    if (!user) return;
    // O nome do canal inclui um id de instância para evitar colisões em
    // strict-mode (React 18 invoca o efeito duas vezes). Sem o sufixo, o
    // supabase realtime cliente devolve o canal já subscribed e o segundo
