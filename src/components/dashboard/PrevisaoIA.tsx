  ...
  const gerarAnalise = async () => {
    ...
    try {
      const { data, error } = await supabase.functions.invoke('analise-preditiva', {
        body: { empresa_id: currentEmpresaId },
      });
    ...
    return (
      <Card className={cn(className, 'premium-card border-none overflow-hidden')}>
        <CardContent className="flex flex-col items-center justify-center py-20 text-center relative overflow-hidden">
          ...
          <h3 className="mb-3 text-2xl font-black text-foreground tracking-tight font-heading relative z-10">
            IA Estratégica P14
          </h3>
          <p className="mb-10 max-w-sm text-sm font-medium text-muted-foreground leading-relaxed px-6 relative z-10">
            Libere o poder da inteligência preditiva para antecipar riscos e otimizar seu fluxo de
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <CardDescription className="text-xs">
                Gerando previsões com inteligência artificial.
              </CardDescription>
            </div>
          </div>
        </CardHeader>
        ...
        <CardHeader className="p-8 pb-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <div>
                <CardTitle className="text-xl font-black text-foreground tracking-tight font-heading">
                  Insights de IA
                </CardTitle>
                {geradoEm && (
                  <CardDescription className="text-[10px] font-black uppercase tracking-[0.15em] text-muted-foreground/60">
                    Análise gerada em {new Date(geradoEm).toLocaleTimeString('pt-BR')}
                  </CardDescription>
                )}
              </div>
            </div>
        ...
        <CardContent className="p-8 pt-2 space-y-8">
                ...
                <div className="flex items-baseline justify-center sm:justify-start gap-1">
                  <span
                    className={cn(
                      ...
                      getScoreColor(analise?.score_saude_financeira || '0')
                    )}
                  >
          ...
          <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
            <TabsList className="grid grid-cols-4 h-9 items-center justify-center rounded-lg bg-muted/50 p-0.5 text-muted-foreground w-full border border-border mb-6">
              <TabsTrigger
                value="visao-geral"
                className="rounded-md data-[state=active]:bg-card data-[state=active]:text-primary font-bold text-[10px] gap-1.5 h-8"
              >
