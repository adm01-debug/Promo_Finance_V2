...
function TendenciaCard({
  ...
  data: AnaliseTendencia;
  colorFn: (t: string, inv?: boolean) => string;
  iconFn: (t: string) => React.ReactNode;
  inverted?: boolean;
}) {
  return (
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-6 md:grid-cols-2">
        <TendenciaCard
          title="Receita Strategy"
          icon={<TrendingUp className="h-4 w-4 text-success" />}
          data={tendencias.receitas}
          colorFn={getTendenciaColor}
          iconFn={getTendenciaIcon}
        />
        <TendenciaCard
          title="Exposição Strategy"
          icon={<TrendingDown className="h-4 w-4 text-streak" />}
          data={tendencias.despesas}
          colorFn={getTendenciaColor}
          iconFn={getTendenciaIcon}
          inverted
        />
        <TendenciaCard
          title="Delinquency Tendency"
          icon={<AlertTriangle className="h-4 w-4 text-destructive" />}
          data={tendencias.inadimplencia}
          colorFn={getTendenciaColor}
          iconFn={getTendenciaIcon}
          inverted
        />
          ...
          <Card className="border-white/5 bg-black/20 backdrop-blur-xl overflow-hidden group">
            ...
            <CardContent className="p-5 space-y-4">
              <div className="flex items-center justify-between">
                ...
                <span className="font-black text-3xl tracking-tighter text-blue-500">
                  {tendencias.margem_liquida?.atual}
                </span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground/50">
                ...
                <div
                  className={`flex items-center gap-2 ${getTendenciaColor(tendencias.margem_liquida?.tendencia)}`}
                >
                  {getTendenciaIcon(tendencias.margem_liquida?.tendencia)}
                  <span className="font-black tracking-tight capitalize">
                    {tendencias.margem_liquida?.tendencia}
                  </span>
                </div>
              </div>
              <div className="pt-4 border-t border-white/5">
                <p className="text-[10px] font-medium text-muted-foreground/70 leading-relaxed italic">
                  {tendencias.margem_liquida?.previsao}
                </p>
              </div>
            </CardContent>
          </Card>
