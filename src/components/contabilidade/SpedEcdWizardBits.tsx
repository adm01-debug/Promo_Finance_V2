...
import { STEPS, type Step } from './SpedEcdWizardConfig';

export type { Step } from './SpedEcdWizardConfig';
...
export function MetaField({ icon: Icon, label, value, mono }: { icon: React.ComponentType<{ className?: string }>; label: string; value: string; mono?: boolean }) {
  return (
...
export function KpiChip({ label, value, tone, Icon }: { label: string; value: number; tone: 'success' | 'destructive' | 'warning'; Icon: React.ComponentType<{ className?: string }> }) {
  const toneClass =
