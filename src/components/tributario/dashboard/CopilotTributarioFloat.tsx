...
const SUGESTOES = [
  'Qual meu regime tributário ideal?',
  'Quais são as 3 maiores oportunidades de economia?',
  'Como está minha conformidade fiscal este mês?',
  'Resuma a Reforma Tributária para minha empresa.',
];

const STORAGE_KEY = 'copilot_tributario_history_v1';

// Sanitização simples de markdown → HTML seguro
...
export function CopilotTributarioFloat({ empresaId }: Props) {
  ...
  const enviar = async (texto: string) => {
    ...
    try {
      ...
      if ((err as Error).name !== 'AbortError') {
        logger.error('Copilot tributário falhou', err as Error);
  ...
  return (
    <Sheet open={open} onOpenChange={setOpen}>
      <SheetTrigger asChild>
        <Button
          size="lg"
          className="fixed bottom-6 right-6 z-50 rounded-full h-14 w-14 shadow-2xl bg-gradient-to-br from-primary to-primary/70 hover:scale-105 transition-transform p-0"
          aria-label="Abrir Copilot Tributário"
        >
        ...
        <SheetHeader className="p-4 border-b">
          <div className="flex items-center justify-between">
            <SheetTitle className="flex items-center gap-2">
              <Bot className="h-5 w-5 text-primary" /> Copilot Tributário
            </SheetTitle>
            {messages.length > 0 && (
          ...
          <p className="text-xs text-muted-foreground text-left">
            Especialista em CBS/IBS/IS, Reforma Tributária e LC 214/25.
          </p>
        </SheetHeader>
