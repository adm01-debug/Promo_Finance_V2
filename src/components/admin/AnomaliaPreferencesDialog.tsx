...
export function AnomaliaPreferencesDialog({ open, onOpenChange }: Props) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const ccsMudou =
        previaCcsSorted.join("|") !== ccsSorted.join("|");
      const tiposMudou =
        previaTiposSorted.join("|") !== tiposSorted.join("|");
      ...
      // Auditoria: registra apenas quando há ação de silenciamento relevante
      if (silenciarAteMudou || ccsMudou || tiposMudou) {
        const agora = new Date();
        const ate = silenciarAte ? new Date(silenciarAte) : null;
        const duracaoMin =
          ate && ate > agora
            ? Math.round((ate.getTime() - agora.getTime()) / 60000)
            : 0;

        const action = ate && ate > agora ? "UPDATE" : "UPDATE";
        const acaoLabel =
          ate && ate > agora
            ? "SILENCE_ALERTS"
            : previaSilenciarAte
