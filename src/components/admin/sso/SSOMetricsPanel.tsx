...
import { useSSOLoginAttempts, useSSOProviders } from '@/hooks/useSSO';
...
export function SSOMetricsPanel() {
  ...
  const stats = useMemo(() => {
    ...
    const byDay: Record<string, { date: string; success: number; fail: number }> = {};
    for (let i = 6; i >= 0; i--) {
      const d = format(subDays(new Date(), i), 'dd/MM');
      byDay[d] = { date: d, success: 0, fail: 0 };
    }
    last7.forEach(a => {
      const d = format(new Date(a.created_at), 'dd/MM');
      if (byDay[d]) byDay[d][a.success ? 'success' : 'fail']++;
    });
