...
export function RegrasDuplicidadeTab() {
  ...
  const { data: config, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      if (data) {
        const campos = Array.isArray(data.campos_validacao)
          ? (data.campos_validacao as string[])
          : [];
  ...
  const saveMutation = useMutation({
    mutationFn: async ({
      ...
      const { data: perfil } = await supabase.from('profiles').select('empresa_id').single();
      ...
      const { error } = await supabase.from('configuracoes_duplicidade').insert({
    ...
    onError: (error: unknown) => {
      toast.error(
        'Erro ao salvar regras: ' + (error instanceof Error ? error.message : String(error))
      );
    },
  });
  ...
  const fields = [
    {
      ...
      description: 'Detecta se o fornecedor ou CNPJ é o mesmo.',
    },
    { id: 'valor', label: 'Valor Exato', description: 'Compara o valor bruto do lançamento.' },
    {
      id: 'numero_documento',
      label: 'Número do Documento',
      description: 'Chave principal (Nota Fiscal, Fatura, etc).',
    },
    {
      id: 'mes_vencimento',
      label: 'Competência (Mês/Ano)',
      description: 'Evita lançamentos duplicados no mesmo período.',
    },
    {
      id: 'data_vencimento',
      label: 'Data de Vencimento Exata',
      description: 'Restrição mais rígida por dia.',
    },
  ];
  ...
  if (isLoading)
    return <div className="p-8 text-center animate-pulse">Sincronizando motor de regras...</div>;
          ...
          <CardHeader className="bg-card/[0.02] border-b border-white/5 p-8">
            <div className="flex items-center justify-between">
              <div className="space-y-1">
                ...
                <CardDescription>
                  Defina os critérios que o sistema deve usar para bloquear pagamentos suspeitos.
                </CardDescription>
              </div>
              <Badge
                variant="outline"
                className="h-6 px-2 text-[10px] font-black uppercase tracking-widest border-primary/20 text-primary"
              >
          ...
          <CardContent className="p-8 space-y-8">
            <div className="grid gap-6">
              {fields.map((field) => (
                <div
                  key={field.id}
                  className="flex items-start justify-between p-5 rounded-2xl bg-card/[0.03] hover:bg-card/[0.06] transition-all border border-white/5 group"
                >
                  <div className="space-y-1">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-4">
                ...
                <div className="flex items-center justify-between">
                  <div className="space-y-1">
                    ...
                    <p className="text-[10px] text-muted-foreground">
                      Detecta nomes de fornecedores similares (ex: "Apple" vs "Apple Inc").
                    </p>
                  </div>
                  <Switch checked={fuzzyMatching} onCheckedChange={setFuzzyMatching} />
                </div>
              </div>
              ...
              <div className="p-6 rounded-2xl bg-card/[0.03] border border-white/5 space-y-4">
                <div className="space-y-1">
                  ...
                  <p className="text-[10px] text-muted-foreground">
                    Janela de dias para considerar um pagamento como duplicado.
                  </p>
                </div>
                <Input
                  type="number"
            ...
            <div className="p-6 rounded-2xl bg-primary/5 border border-primary/10 flex items-start gap-4">
              ...
              <div className="space-y-1">
                ...
                <p className="text-xs text-primary/70 leading-relaxed">
                  O sistema valida tokens de idempotência em todas as APIs críticas de pagamento
                  10/10.
                </p>
              </div>
            </div>
          ...
          <Card className="border-none bg-primary shadow-[0_20px_50px_rgba(var(--primary),0.3)] rounded-[2rem] text-primary-foreground overflow-hidden group">
            ...
            <CardContent className="p-8 pt-0 space-y-6">
              <p className="text-sm text-foreground/80 leading-relaxed">
                As alterações nestas regras são aplicadas instantaneamente a todos os novos
                lançamentos e edições.
              </p>
              <Button
                onClick={() =>
                  saveMutation.mutate({
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between text-xs">
                  ...
                  <span className="font-bold">
                    {config?.created_at ? format(new Date(config.created_at), 'dd/MM/yy') : '--/--'}
                  </span>
                </div>
