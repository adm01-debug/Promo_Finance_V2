      ...
      <>
        ...
        <TableCell className="p-6">
          <div className="flex items-center gap-4">
            ...
            <div className="min-w-0">
              <p className="font-black text-base tracking-tight text-foreground truncate">
                {conta.fornecedor_nome}
              </p>
              <div className="flex items-center gap-2 mt-0.5">
                <TipoIcon className="h-3 w-3 text-muted-foreground/40" />
                <span className="text-[10px] font-black uppercase tracking-widest text-muted-foreground/40 capitalize">
                  {conta.tipo_cobranca}
                </span>
              </div>
            </div>
          ...
          <div className="space-y-1.5">
              ...
              <CategorizacaoIABadge
                ...
                onAplicar={async (cat) => {
                  const { error } = await supabase
                    .from('contas_pagar')
                    // TODO: coluna 'tags' não existe em contas_pagar no types.ts (removida)
                    .update({ categoria: cat.categoria })
                    .eq('id', conta.id);
                  if (error) {
            ...
            <div className="flex items-center gap-2">
              {conta.categoria && (
                <Badge
                  variant="outline"
                  className="text-[9px] font-black uppercase px-1.5 py-0 rounded-md border-secondary/20 bg-secondary/5 text-secondary tracking-wider"
                >
            ...
            <div
              className={cn(
                ...
                overdueDays > 0 && conta.status !== 'pago'
                  ...
                  : 'bg-card/5 border-white/10 text-muted-foreground/40'
              )}
            >
        ...
        <TableCell className="p-6">
          {conta.centro_custo_nome || conta.centros_custo?.nome ? (
            <Badge
              variant="outline"
              className="text-[10px] font-black uppercase tracking-[0.15em] text-muted-foreground/60 border-white/5 bg-card/5 px-2 py-0.5"
            >
              {conta.centro_custo_nome || conta.centros_custo?.nome}
            </Badge>
          ) : (
        ...
        <TableCell className="p-6 text-center">
          <Badge
            variant="outline"
            className={cn(
              'gap-1.5 px-3 py-1 rounded-lg border-none font-black text-[10px] uppercase tracking-widest shadow-sm',
              status?.color
            )}
          >
        ...
        <TableCell className="p-6 text-right">
          <DropdownMenu>
            ...
            <DropdownMenuContent
              align="end"
              className="w-56 bg-background/95 backdrop-blur-xl border-white/10 rounded-xl"
            >
              ...
              {aguardandoSolicitacao && (
                <DropdownMenuItem
                  ...
                  className="gap-2 text-warning focus:text-warning focus:bg-warning/10"
                >
