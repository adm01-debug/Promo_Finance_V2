...
import {
  ...
  QrCode, Wallet, Link2, User, Edit, RefreshCw, Layers, Search,
} from 'lucide-react';
...
export function ContaReceberForm({ open, onOpenChange, conta }: ContaReceberFormProps) {
  ...
  useEffect(() => {
    if (conta && open) {
      form.reset({
        ...
        recorrente: false, parcelado: false, numero_parcelas: 2,
      });
  ...
  const createMutation = useMutation({
    mutationFn: async (data: ContaReceberFormData) => {
      ...
      // Parcelamento (#8): create N installments
      if (data.parcelado && data.numero_parcelas && data.numero_parcelas > 1) {
        ...
        for (let i = 0; i < data.numero_parcelas; i++) {
          ...
          parcelas.push({
            ...
            valor: i === data.numero_parcelas - 1
              ? Math.round((data.valor - valorParc * (data.numero_parcelas - 1)) * 100) / 100
              : valorParc,
            ...
            etapa_cobranca: 'preventiva' as const,
      ...
      // Single insert (with recorrente flag #7)
      ...
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
            {/* Cliente with fuzzy autocomplete (#6) */}
            ...
            {/* Recorrência (#7) + Parcelamento (#8) — only for new */}
            {!isEditing && (
                    ...
                    <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }}>
                      <FormField control={form.control} name="frequencia_recorrencia" render={({ field }) => (
                        <Select onValueChange={field.onChange} value={field.value || 'mensal'}>
                          ...
                          <SelectContent>
                            {frequenciaOptions.map(f => <SelectItem key={f.value} value={f.value}>{f.label}</SelectItem>)}
                          </SelectContent>
                        </Select>
                      )} />
                    </motion.div>
                    ...
                    <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} className="space-y-2">
                      ...
                      {valorTotal > 0 && (
                        <p className="text-xs text-muted-foreground">
                          {numParcelas}x de <span className="font-semibold text-foreground">R$ {valorParcela.toFixed(2)}</span>
                        </p>
                      )}
                    </motion.div>
                  )}
