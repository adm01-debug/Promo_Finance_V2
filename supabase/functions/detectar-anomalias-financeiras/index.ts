...
serve(async (req) => {
  ...
  const triggerSource = req.headers.get('x-trigger-source') === 'cron' ? 'cron' : 'manual';
  ...
  const { data: lockRow, error: lockErr } =
    (await client
      .rpc(
      ...
      .single?.()) ?? ({ data: null, error: null } as { data: unknown; error: unknown });
  void lockRow;
  void lockErr;
  ...
  if (body.run_id) {
    ...
    const { data: newRun, error: createErr } = await client
      ...
      .single();
    if (createErr || !newRun) {
      return new Response(JSON.stringify({ error: createErr?.message ?? 'failed_to_create_run' }), {
        status: 500,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      });
    }
  ...
  try {
    ...
    const { data: empresas } = await client.from('empresas').select('id').eq('ativo', true);
    ...
    if (regimes) {
      const byEmp = new Map<
        string,
        Array<{ carga_pct: number | null; ano: number; mes: number }>
      >();
      ...
      for (const [empId, arr] of byEmp.entries()) {
        ...
        if (atual.carga_pct && anterior.carga_pct) {
          const variacao = Math.abs(
            ((Number(atual.carga_pct) - Number(anterior.carga_pct)) / Number(anterior.carga_pct)) *
              100
          );
          if (variacao > 30) {
            novas.push({
              ...
              dados: {
                ...
                atual: atual.carga_pct,
                anterior: anterior.carga_pct,
              },
            });
          }
        }
