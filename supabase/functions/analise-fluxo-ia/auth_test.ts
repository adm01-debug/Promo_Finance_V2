...
Deno.test({
  name:
    "funções de IA rejeitam chamadas anônimas e JWT inválido antes de IA/DB",
  sanitizeOps: false,
  sanitizeResources: false,
  async fn(t) {
    const fetchOriginal = globalThis.fetch;
    const ambienteOriginal = new Map(
      CHAVES_AMBIENTE.map((chave) => [chave, Deno.env.get(chave)]),
    );
    ...
    try {
      for (const funcao of FUNCOES) {
        ...
        await t.step(
          `${funcao.nome}: 401 com anon key como Bearer`,
          async () => {
            globalThis.fetch = () => {
            ...
            const response = await funcao.handler(
              requisicao(funcao.nome, "Bearer anon-test-key"),
            );
        ...
        await t.step(
          `${funcao.nome}: 401 com JWT inválido sem tocar IA/DB`,
          async () => {
            const urls: string[] = [];
            ...
            const response = await funcao.handler(
              requisicao(funcao.nome, "Bearer jwt-invalido"),
            );
            ...
            assertEquals(urls.some((url) => url.includes("ai.gateway")), false);
            assertEquals(urls.some((url) => url.includes("/rest/v1/")), false);
          },
        );
      }
    } finally {
