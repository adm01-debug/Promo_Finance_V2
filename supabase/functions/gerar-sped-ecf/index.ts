...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const parsedBody = await parseJsonBody(req, corsHeaders, 'gerar-sped-ecf');
    if (!parsedBody.success) return parsedBody.response;
    const __contract = validatePayload(z.object({ empresa_id: z.string().uuid(), ano_calendario: z.number().int().min(2000).max(2100), mode: z.enum(['validate','generate']).optional() }), parsedBody.data, 'gerar-sped-ecf');
