...
Deno.test("avaliarSqlMcp bloqueia classes perigosas mesmo com allow_all_rows", () => {
  for (const sql of bloqueadosSempre) {
    ...
    assertEquals(
      typeof avaliacao.motivoBloqueio,
      "string",
...
Deno.test("avaliarSqlMcp só libera UPDATE/DELETE sem escopo com allow_all_rows:true", () => {
  ...
  assertEquals(
    typeof avaliarSqlMcp(updateSemWhere, false).motivoBloqueio,
    "string",
  );
  assertEquals(
    typeof avaliarSqlMcp(deleteSemWhere, false).motivoBloqueio,
    "string",
  );
...
Deno.test("aplicarLimitePadrao sempre aplica cap externo em leituras", () => {
  ...
  const comLimite = aplicarLimitePadrao(
    "SELECT * FROM public.empresas LIMIT 10",
    321,
  );
  ...
  const escrita = aplicarLimitePadrao(
    "UPDATE public.empresas SET nome = 'x' WHERE id = 1",
    321,
  );
