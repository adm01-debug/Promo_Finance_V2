...
Deno.test("handler aplica timeout também em escrita única", async () => {
  ...
  assertEquals(client.calls[2], "SET LOCAL lock_timeout = '1000ms'");
  assertEquals(
    client.calls.includes("SET TRANSACTION READ ONLY"),
    false,
  );
  assertEquals(
    client.calls[3],
...
Deno.test("handler rejeita content-length inválido antes do parse", async () => {
  ...
  const response = await handler(
    makeRawRequest('{"sql":"SELECT 1"}', { "content-length": "abc" }),
  );
  const payload = await response.json();
...
Deno.test("handler exige exatamente um modo por request", async () => {
  ...
  const respostaComDoisModos = await handler(makeRequest({
    sql: "SELECT 1",
    admin: { path: "auth/v1/admin/users", method: "GET" },
  }));
  const respostaSqlDuplicado = await handler(makeRequest({
    sql: "SELECT 1",
...
Deno.test("handler rejeita admin.path com dot segments, método arbitrário e rota fora da allowlist", async () => {
  const handler = createHandler({
    secret: "segredo-correto",
    serviceKey: "srv",
  ...
  const respostaRota = await handler(makeRequest({
...
Deno.test("handler limita body e resposta do proxy admin", async () => {
  ...
  const respostaBody = await handler(makeRequest({
    admin: {
  ...
  const respostaResposta = await handler(makeRequest({
    admin: { path: "auth/v1/admin/users", method: "GET" },
  }));
...
Deno.test("handler aceita rotas admin allowlisted realmente usadas pelo worker", async () => {
  ...
  const respostaEsvaziarBucket = await handler(makeRequest({
    admin: { path: "storage/v1/bucket/arquivos/empty", method: "POST" },
  }));
  const respostaDeletePrefixos = await handler(makeRequest({
    admin: {
...
Deno.test("handler exige contrato explícito para DELETE storage/v1/object/<bucket>", async () => {
  ...
  const semPrefixes = await handler(makeRequest({
    admin: { path: "storage/v1/object/arquivos", method: "DELETE" },
  }));
  const prefixosInvalidos = await handler(makeRequest({
    admin: {
      path: "storage/v1/object/arquivos",
      method: "DELETE",
      body: { prefixes: ["", 1] },
