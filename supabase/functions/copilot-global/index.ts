...
async function executeTool(name: string, sb: ReturnType<typeof createClient>, userId: string) {
  try {
    if (name === 'consultar_kpis_financeiros') {
      const [{ data: cp }, { data: cr }, { data: contas }] = await Promise.all([
        sb
          .from('contas_pagar')
          .select('valor,status')
          .in('status', ['pendente', 'vencido', 'atrasado']),
        sb
          .from('contas_receber')
          .select('valor,status')
          .in('status', ['pendente', 'vencido', 'atrasado']),
      ...
      const saldoTotal = (contas ?? []).reduce(
        (s: number, x: any) => s + Number(x.saldo_atual || 0),
        0
      );
      return {
        totalPagar,
        totalReceber,
        saldoTotal,
        saldoLiquido: saldoTotal + totalReceber - totalPagar,
      };
    }
    if (name === 'listar_acoes_recomendadas') {
      const { data } = await sb
        .from('acoes_recomendadas')
        .select('titulo,descricao,urgencia,impacto_estimado')
    ...
    if (name === 'buscar_alertas_criticos') {
      const { data } = await sb
        .from('alertas')
        .select('titulo,mensagem,prioridade,created_at')
        .eq('user_id', userId)
        .eq('lido', false)
