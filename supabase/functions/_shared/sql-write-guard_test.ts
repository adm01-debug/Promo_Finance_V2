...
Deno.test('analisarSqlMcp permite leituras SELECT/WITH somente leitura', () => {
  for (const sql of leiturasPermitidas) {
    ...
    assertEquals(analise.somenteLeitura, true, `deveria marcar como leitura: ${sql}`);
  }
});
...
Deno.test(
  'analisarSqlMcp bloqueia comandos perigosos, funções laterais e quoted identifiers',
  () => {
    for (const sql of bloqueiosSempre) {
      const analise = analisarSqlMcp(sql);
      assertEquals(typeof analise.motivoBloqueio, 'string', `deveria bloquear: ${sql}`);
    }
  }
);
...
Deno.test('validarEscritaEscopada exige escopo verificável em UPDATE/DELETE', () => {
  for (const sql of bloqueiosEscopo) {
    ...
    assertEquals(typeof motivo, 'string', `deveria bloquear por escopo: ${sql}`);
  }
});
