...
function fakeSupabaseClient(
    ...
    from(table: string) {
      ...
      const resultForSelect = () => {
        ...
        if (table === 'contas_receber') {
          const id = filters.find((f) => f.method === 'eq' && f.column === 'id')?.value;
          if (typeof id === 'string') {
            return database.contasPorId?.[id] ?? { data: null, error: null };
          }
          return filters.some((f) => f.method === 'lt' && f.column === 'data_vencimento')
  ...
  return client as unknown as ReturnType<WhatsappIaProativoDependencies['criarClient']>;
}
...
Deno.test(
  'whatsapp-ia-proativo: resultado usuário do guard mantém o fluxo do frontend',
  async () => {
    const calls = counters();
    const handler = createHandler(dependencies(calls));
    const response = await handler(
...
Deno.test(
  'whatsapp-ia-proativo: empresa alheia falha antes de rate limit, IA ou link',
  async () => {
    const calls = counters();
    ...
    assertEquals(
      dbCalls.map((call) => call.table),
      ['user_empresas']
    );
    assertEquals(dbCalls[0].filters, [
...
Deno.test(
  'whatsapp-ia-proativo: usuário sem empresa ativa falha fechado antes do link',
  async () => {
    const calls = counters();
    const handler = createHandler(
      dependencies(calls, {}, { userEmpresas: { data: [], error: null } })
    );
    const response = await handler(
      post({
        action: 'test',
...
Deno.test(
  'whatsapp-ia-proativo: consultas de alertas filtram contas e clientes pelo tenant',
  async () => {
    await withAuthEnvironment(async () => {
      const calls = counters();
      const dbCalls: DbCall[] = [];
      ...
      const response = await handler(post({ action: 'analisar-alertas', data: {} }));
      ...
      const consultasTenant = dbCalls.filter(
        (call) => call.table === 'contas_receber' || call.table === 'clientes'
      );
...
Deno.test('whatsapp-ia-proativo: job interno sem empresa preserva escopo global', async () => {
  await withAuthEnvironment(async () => {
    ...
    const handler = createHandler(deps);
    const response = await handler(
      post({ action: 'analisar-alertas', data: {} }, { Authorization: 'Bearer service-role-teste' })
    );
    ...
    const consultasNegocio = dbCalls.filter(
      (call) => call.table === 'contas_receber' || call.table === 'clientes'
    );
...
Deno.test(
  'whatsapp-ia-proativo: contrato legado continua válido para usuário autorizado',
  async () => {
    const calls = counters();
    const handler = createHandler(dependencies(calls));
    const response = await handler(
