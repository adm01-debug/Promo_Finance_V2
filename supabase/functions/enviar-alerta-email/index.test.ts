...
Deno.test(
  'enviar-alerta-email rejeita chamada sem credencial antes de acessar serviços',
  async () => {
...
Deno.test(
  'enviar-alerta-email rejeita segredo interno incorreto antes de acessar serviços',
  async () => {
    const response = await executarSemRede(
      new Request('http://localhost/enviar-alerta-email', {
        method: 'POST',
