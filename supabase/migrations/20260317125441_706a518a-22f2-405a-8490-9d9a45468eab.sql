...
-- vw_contas_pagar_painel: guard on conta_bancaria_id (adicionada em migracao posterior ao schema base).
-- No Preview incremental essa coluna pode nao existir ainda; pula a criacao e deixa migracao posterior recriar.
DO $$
BEGIN
  IF EXISTS (
    ...
    $view$;
  ELSE
    RAISE NOTICE '20260317125441: contas_pagar.conta_bancaria_id ausente; vw_contas_pagar_painel sera recriada em migracao posterior.';
  END IF;
END
$$;

-- vw_contas_receber_painel: guard on conta_bancaria_id (mesmo padrao que contas_pagar).
DO $$
BEGIN
  IF EXISTS (
    SELECT 1 FROM information_schema.columns
    ...
    $view$;
  ELSE
    RAISE NOTICE '20260317125441: contas_receber.conta_bancaria_id ausente; vw_contas_receber_painel sera recriada em migracao posterior.';
  END IF;
END
$$;

-- vw_dre_mensal: guard on movimentacoes.empresa_id (coluna multi-empresa adicionada posteriormente).
DO $$
BEGIN
  IF EXISTS (
    SELECT 1 FROM information_schema.columns
    ...
    $view$;
  ELSE
    RAISE NOTICE '20260317125441: movimentacoes.empresa_id ausente; vw_dre_mensal sera recriada em migracao posterior.';
  END IF;
END
$$;

-- vw_dso_aging: guard duplo em contas_receber.empresa_id e valor_recebido (colunas ausentes no Preview incremental).
DO $$
BEGIN
  IF EXISTS (
    SELECT 1 FROM information_schema.columns
    ...
    $view$;
  ELSE
    RAISE NOTICE '20260317125441: contas_receber.empresa_id ou valor_recebido ausente; vw_dso_aging sera recriada em migracao posterior.';
  END IF;
END
$$;

-- vw_saldos_contas: guard on contas_bancarias.empresa_id (coluna multi-empresa).
DO $$
BEGIN
  IF EXISTS (
    SELECT 1 FROM information_schema.columns
    ...
    $view$;
  ELSE
    RAISE NOTICE '20260317125441: contas_bancarias.empresa_id ausente; vw_saldos_contas sera recriada em migracao posterior.';
  END IF;
END
$$;

-- vw_fluxo_caixa: guard on movimentacoes.empresa_id.
-- Essa view eh recreada tambem em 20260904000600 quando valor_pago existe.
DO $$
BEGIN
  IF EXISTS (
    SELECT 1 FROM information_schema.columns
    ...
    $view$;
  ELSE
    RAISE NOTICE '20260317125441: movimentacoes.empresa_id ausente; vw_fluxo_caixa sera recriada em migracao posterior.';
  END IF;
END
$$;

-- vw_fluxo_caixa_diario: guard on movimentacoes.empresa_id.
DO $$
BEGIN
  IF EXISTS (
    SELECT 1 FROM information_schema.columns
    ...
    $view$;
  ELSE
    RAISE NOTICE '20260317125441: movimentacoes.empresa_id ausente; vw_fluxo_caixa_diario sera recriada em migracao posterior.';
  END IF;
END
$$;

-- vw_gastos_centro_custo: guard on centros_custo.bitrix_deal_id (integracao Bitrix adicionada posteriormente).
DO $$
BEGIN
  IF EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_schema = 'public' AND table_name = 'centros_custo' AND column_name = 'bitrix_deal_id'
  ) THEN
    EXECUTE $view$
    ...
    $view$;
  ELSE
    RAISE NOTICE '20260317125441: centros_custo.bitrix_deal_id ausente; vw_gastos_centro_custo sera recriada em migracao posterior.';
  END IF;
END
$$;

-- vw_metricas_cobranca: guard on tabela execucoes_cobranca (modulo de cobranca adicionado posteriormente).
DO $$
BEGIN
  IF EXISTS (
    ...
    $view$;
  ELSE
    RAISE NOTICE '20260317125441: tabela execucoes_cobranca ausente; vw_metricas_cobranca sera recriada em migracao posterior.';
  END IF;
END
$$;

-- vw_transferencias_painel: guard on transferencias.modalidade (coluna adicionada posteriormente).
DO $$
BEGIN
  IF EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_schema = 'public' AND table_name = 'transferencias' AND column_name = 'modalidade'
  ) THEN
    EXECUTE $view$
    ...
    $view$;
  ELSE
    RAISE NOTICE '20260317125441: transferencias.modalidade ausente; vw_transferencias_painel sera recriada em migracao posterior.';
  END IF;
END
$$;

-- vw_webhooks_recentes: guard on webhooks_log.evento (coluna adicionada posteriormente).
DO $$
BEGIN
  IF EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_schema = 'public' AND table_name = 'webhooks_log' AND column_name = 'evento'
  ) THEN
    EXECUTE $view$
    ...
    $view$;
  ELSE
    RAISE NOTICE '20260317125441: webhooks_log.evento ausente; vw_webhooks_recentes sera recriada em migracao posterior.';
  END IF;
END
$$;
