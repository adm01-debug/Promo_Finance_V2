-- As 11 views abaixo podem não existir aqui pelo mesmo motivo de 20260317125502
-- (guards de 20260317125441 sobre colunas só adicionadas em maio; ALTERs fora
-- do guard quebraram o replay do zero — ver PR #88).
DO $$
BEGIN
  ...
  IF to_regclass('public.vw_transferencias_painel') IS NOT NULL THEN
    EXECUTE 'ALTER VIEW public.vw_transferencias_painel SET (security_invoker = true)';
  END IF;
  IF to_regclass('public.vw_saldos_contas') IS NOT NULL THEN
    EXECUTE 'ALTER VIEW public.vw_saldos_contas SET (security_invoker = true)';
  END IF;
  IF to_regclass('public.vw_fluxo_caixa') IS NOT NULL THEN
    EXECUTE 'ALTER VIEW public.vw_fluxo_caixa SET (security_invoker = true)';
  END IF;
  IF to_regclass('public.vw_fluxo_caixa_diario') IS NOT NULL THEN
    EXECUTE 'ALTER VIEW public.vw_fluxo_caixa_diario SET (security_invoker = true)';
  END IF;
  IF to_regclass('public.vw_metricas_cobranca') IS NOT NULL THEN
    EXECUTE 'ALTER VIEW public.vw_metricas_cobranca SET (security_invoker = true)';
  END IF;
  IF to_regclass('public.vw_webhooks_recentes') IS NOT NULL THEN
    EXECUTE 'ALTER VIEW public.vw_webhooks_recentes SET (security_invoker = true)';
  END IF;
END
$$;