...
-- As 11 views abaixo podem não existir aqui: os guards de 20260317125441
...
-- os ALTERs fora do guard quebraram o Supabase Preview do PR #88 de novo).
DO $$
BEGIN
  ...
  IF to_regclass('public.vw_transferencias_painel') IS NOT NULL THEN
    EXECUTE 'ALTER VIEW public.vw_transferencias_painel SET (security_invoker = on)';
  END IF;
  IF to_regclass('public.vw_webhooks_recentes') IS NOT NULL THEN
    EXECUTE 'ALTER VIEW public.vw_webhooks_recentes SET (security_invoker = on)';
  END IF;
END
$$;
