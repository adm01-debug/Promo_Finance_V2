-- Migration 20260903000700
-- PROBLEMA: auditar_acessos_cross_tenant() usa COALESCE(ue.ativo, true) no CTE "viol"
-- para detectar acessos suspeitos. Após a correção de empresa_acessivel() (migration
-- 20260903000600), usuários com ativo=NULL não têm mais acesso, mas o detector ainda
-- os trata como membros ativos → não os flagra como cross_tenant. Inconsistência.
-- FIX: substituir COALESCE(ue.ativo, true) por ue.ativo = true (alinha com empresa_acessivel).
  ...
  WITH base AS (
    ...
    WHERE b.empresa_id IS NOT NULL
      AND NOT EXISTS (
        ...
        WHERE ue.user_id = b.user_id AND ue.empresa_id = b.empresa_id AND ue.ativo = true
      )
  ), ins AS (
