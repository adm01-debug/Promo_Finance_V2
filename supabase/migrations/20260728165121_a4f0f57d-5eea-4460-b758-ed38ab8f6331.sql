...
BEGIN
  WITH titulos AS (
    ...
    UNION ALL
    SELECT cr.id, cr.empresa_id, COALESCE(cr.numero_documento, 'TÃ­tulo a receber'), cr.valor,
    ...
    SELECT d.user_id,
           ...
           CASE WHEN d.data_vencimento < CURRENT_DATE
                THEN 'TÃ­tulo vencido' ELSE 'Vencimento prÃ³ximo' END,
           format('%s - R$ %s com vencimento em %s',
                  COALESCE(d.titulo_desc, 'TÃ­tulo'),
...
BEGIN
  IF p_empresa_id IS NULL THEN
    RAISE EXCEPTION 'empresa_id Ã© obrigatÃ³rio';
  END IF;
  IF p_tabela NOT IN ('contas_pagar', 'contas_receber') THEN
    RAISE EXCEPTION 'Tabela invÃ¡lida: %. Use contas_pagar ou contas_receber.', p_tabela;
  END IF;
  IF NOT public.empresa_acessivel(p_empresa_id) THEN
    RAISE EXCEPTION 'Acesso negado Ã  empresa informada';
  END IF;
